...
export default class ColorToCssSolver {
  public target: RGBColor;
  public targetHSL: {h: number, s: number, l: number};
  public reusedColor: RGBColor;

  public static colorToCssFilter(hex: string): string {
    const rgb = hexToRgb(hex);
    const color = new RGBColor(rgb[0], rgb[1], rgb[2]);
    const solver = new ColorToCssSolver(color);
    return solver.solve().filter;
  }

  constructor(target: RGBColor) {
    this.target = target;
