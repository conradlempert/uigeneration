...
export default class ConstraintShapeDetection extends AbstractShapeDetection {
  public static async detectShapes(
  ...
  public static async findLevel1AutoShapes(
    documents: SvgDocument[]
  ): Promise<AutoShape[]> {
    ...
    for (const [groupName, shapes] of topologies)
      groupShapes.set(
        groupName,
        shapes.map((s) => [s])
      );
  ...
  public static async findLevel2AutoShapes(
    documents: SvgDocument[],
    maxL1Shapes = 4
  ): Promise<AutoShape[]> {
    ...
    for (const document of documents) {
      ...
      while ((subset = gen.next())) {
        ...
        for (const permutation of this.getPermutations(subset)) {
          ...
          const topologyName = permutation
            .map((x) => shapeTopologies.get(x))
  ...
  private static getPermutations(arr: any[]): any[][] {
    ...
    return arr.reduce(
      (acc, item, i) =>
        acc.concat(
          this.getPermutations([...arr.slice(0, i), ...arr.slice(i + 1)]).map(
            (val) => [item, ...val]
          )
        ),
      []
  ...
  private static async findAutoShapesInGroups(
    groups: Map<string, PathShape[][]>,
    groupsPointData: Map<PathShape[], Map<Vector2, string>[]>,
    level: 1 | 2
  ) {
    ...
    for (const groupName of groups.keys()) {
      ...
      let currentSize = 1;
      while (currentSize < groupShapes.length) {
        // brute force all combinations (can this be done not exponentially?)
        const currentElements = Array.from(groupConstraints.keys()).filter(
          (k) => k.length === currentSize
        );
        for (const element of currentElements) {
          ...
          for (const shapes of otherShapes) {
            ...
            const newConstraints = this.filterAgainstPoints(
              groupConstraints.get(element),
              groupsPointData.get(shapes)
            );
      ...
      for (const [groupShapes, constraints] of groupConstraints.entries()) {
        const exampleShape = this.findRepresentativeExampleShape(
          groupShapes,
          Array.from(groupConstraints.keys())
        );
        ...
        autoShape.shapeName =
          groupName + "_" + autoShape.cost + "_" + randomstring.generate(4);
      ...
      for (const autoShape of topologyAutoShapes) {
        for (const autoShape2 of topologyAutoShapes) {
          ...
          const pathShapes1 = groupShapes.filter((s) =>
            s[0].representations.some((r) => r.autoShape === autoShape)
          );
          const pathShapes2 = groupShapes.filter((s) =>
            s[0].representations.some((r) => r.autoShape === autoShape2)
          );
          if (
            pathShapes1.length === union(pathShapes1, pathShapes2).length &&
            pathShapes1.length > pathShapes2.length
          ) {
  ...
  public static jSketcherIndexFromPointIndex(
    pointIndex: number,
    shape: PathShape
  ) {}
  ...
  public static computeDOFHeuristic(autoShape: AutoShape): void {
    ...
    const positionConstraints = minimized.filter(
      (c) => c.description === "position"
    ).length;
    const slopeConstraints = minimized.filter(
      (c) => c.description === "slope"
    ).length;
  ...
  public static async removeRedundantConstraintsAndComputeDOF(
    ...
    //   const redundant = Array.from(
    //     UIGenerationMenu.viewer.parametricManager.algNumSystem.redundant
    //   );
    //   const allConstraints = Array.from(
    //     UIGenerationMenu.viewer.parametricManager.algNumSystem.allConstraints
    //   );

    //   console.log(
    //     "AutoShape constraints before removing redundant:",
    //     autoShape.constraints
    //   );
    ...
    //   const isolationDOF =
    //     UIGenerationMenu.viewer.parametricManager.algNumSystem.polynomialIsolations.reduce(
    //       (a, b) => a + b.dof,
    //       0
    //     );
  ...
  public static performTransitiveReduction(autoShapes: AutoShape[]): void {
    ...
    for (const autoShape1 of autoShapes) {
      for (const [idx, autoShape2] of autoShape1.specializedBy.entries()) {
        if (
          autoShape1.specializedBy.some(
            (s) => s !== null && hasAsChild(s, autoShape2)
          )
        ) {
          autoShape1.specializedBy[idx] = null;
        }
      }
      autoShape1.specializedBy = autoShape1.specializedBy.filter(
        (s) => s !== null
      );
    }
  }

  public static deduplicateByConstraints(
    groupConstraints: Map<PathShape[][], Constraint[]>,
    pointData: Map<PathShape[], Map<Vector2, string>[]>
  ): void {
  ...
  public static findRepresentativeExampleShape(
    group: PathShape[][],
    allGroups: PathShape[][][]
  ): PathShape[] {
  ...
  public static pathShapeToPoints(shape: PathShape): Map<Vector2, string>[] {
    ...
    for (const startIndex of topologyStartIndices) {
      ...
      jSketcherPrimitives = [
        ...jSketcherPrimitives.slice(startIndex),
        ...jSketcherPrimitives.slice(0, startIndex),
      ];
      jSketcherPrimitives.forEach((primitive, i) => {
        if (primitive.type === "Segment") {
          points.set(
            new Vector2(primitive.data.a.x, primitive.data.a.y),
            i.toString() + ":A"
          );
          points.set(
            new Vector2(primitive.data.b.x, primitive.data.b.y),
            i.toString() + ":B"
          );
        }
        if (primitive.type === "Arc") {
          ...
          points.set(
            new Vector2(primitive.data.c.x, primitive.data.c.y),
            i.toString() + ":C"
          );
        }
      });
  ...
  public static filterAgainstPoints(
    constraints: Constraint[],
    pointsForStartIndices: Map<Vector2, string>[]
  ): Constraint[] {
    ...
    for (const points of pointsForStartIndices) {
      ...
      constraintResults.push(
        constraints.filter((constraint) => constraint.isSatisfied(idsToPoints))
      );
    }
  ...
  public static minimizeConstraints(constraints: Constraint[]): Constraint[] {
    ...
    let constraintPresent = (
      desc: string,
      points: string[],
      needsToBeInSet = false
    ) => {
    ...
    let relativeDistanceConstraintPresent = (
      edgeA: [string, string],
      edgeB: [string, string],
      needsToBeInSet = false
    ) => {
      for (const constraint of constraints) {
        ...
        if (
          (sameEdge(edgeA, relDistConstraint.getFirstEdge()) &&
            sameEdge(edgeB, relDistConstraint.getSecondEdge())) ||
          (sameEdge(edgeB, relDistConstraint.getFirstEdge()) &&
            sameEdge(edgeA, relDistConstraint.getSecondEdge()))
        ) {
          return true;
        }
      }
    ...
    for (const constraint of angleConstraints) {
      if (
        !constraintPresent("distance", [constraint.indexA, constraint.indexB])
      )
        continue;
      if (
        !constraintPresent("distance", [constraint.indexB, constraint.indexP])
      )
        continue;
      if (
        !constraintPresent("distance", [constraint.indexP, constraint.indexA])
      )
        continue;
    ...
    for (const constraint of angleConstraints2) {
      if (!constraintPresent("slope", [constraint.indexB, constraint.indexP]))
        continue;
      if (!constraintPresent("slope", [constraint.indexP, constraint.indexA]))
        continue;
    ...
    for (const constraint of relativeDistanceConstraints) {
      if (
        ...
        continue;
      if (
        !constraintPresent("distance", [constraint.indexC, constraint.indexD])
      )
        continue;
    ...
    for (const constraint of relativeDistanceConstraints) {
      ...
      for (const edge of relativeDistanceRoots) {
        ...
        if (
          relativeDistanceConstraintPresent(edge, edgeA, false) ||
